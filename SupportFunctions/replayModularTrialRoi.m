function I2 = replayModularTrialRoi(p, replayWindow, isGazeContingent, showWinClip, frameIndex, verbose, vidFilename)
...
if nargin < 5 || isempty(frameIndex)
...
if ~isempty(vidFilename)
    vidObj = VideoWriter(fullfile(pwd, vidFilename));
    vidObj.Quality = 100;
    vidObj.FrameRate=p.trial.display.frate;
...
%     I(:,:,:,p.trial.iFrame-(frameIndex(1)-1))=Screen('GetImage', p.trial.display.ptr, p.trial.display.winRect);% %#ok<AGROW>
    ...
    if getWindow(3)<=p.trial.display.winRect(3) && ...
        ...
        try
            if nargout == 0
                I2 = Screen('GetImage', p.trial.display.ptr, getWindow);% %#ok<AGROW>
            else
    ...
    if ~isempty(vidFilename)
        if nargout == 0
            currFrame = struct('cdata', I2, 'colormap', []);
        else
            currFrame = struct('cdata', squeeze( I2(:,:,:,p.trial.iFrame-(frameIndex(1)-1))), 'colormap', []);
        end
